...
class GaussianDiffusionBeatGans:
    ...
    def training_losses(self,
        ...
        if self.loss_type in [
            ...
            style_logits = model_forward.style_pred
            ...
            if font is not None and style_logits is not None:   
                style_pred_loss = F.cross_entropy(style_logits, font.squeeze(), reduction='mean')
                terms["style_loss"] = style_pred_loss
        else:
    ...
    def ddim_sample_loop_progressive(
        ...
        if font != None: 
            if img.shape[0] !=font.shape[0]:
                font = font[:img.shape[0]]
                label = label[:img.shape[0]]
        if progress:
            # Lazy import so that we don't depend on tqdm.
            from tqdm.auto import tqdm
