...
def latent_diffusion128_config(conf: TrainConfig):
    ...
    conf.batch_size_eval = 256
    return conf

def latent_diffusion32_config(conf: TrainConfig):
    conf = latent_diffusion_config(conf)
...
def latent_256_batch_size(conf: TrainConfig):
    ...
    conf.save_every_samples = 2_000_000
    conf.total_samples = 301_00_000
    return conf

def latent_32_batch_size(conf: TrainConfig):
    conf.batch_size = 128
    conf.eval_ema_every_samples = 500
    conf.eval_every_samples = 1000
    conf.sample_every_samples = 500
    conf.save_every_samples = 500
...
def font32_autoenc_latent():
    conf = pretrain_font32_autoenc10K()
    conf = latent_diffusion128_config(conf)
    conf = latent_mlp_2048_norm_10layers(conf)
    conf = latent_256_batch_size(conf)
    conf = adamw_weight_decay(conf)
    ...
    conf.latent_loss_type = LossType.l1
    conf.latent_beta_scheduler = 'const0.008'
    conf.name = 'font32_autoenc_latent'
    return conf
