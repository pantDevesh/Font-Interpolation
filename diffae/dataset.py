...
class BaseLMDBFont(Dataset):
    ...
    def __getitem__(self, index):
        with self.env.begin(write=False) as txn:
            ...
            font_key = f'font-{index:09d}'.encode()
                    
            img_bytes = txn.get(img_key)
            fontname = txn.get(font_key).decode()
            label = txn.get(label_key).decode()

        buffer = BytesIO(img_bytes)
        img = Image.open(buffer)
        return img, fontname, label
...
class FONTlmdb(Dataset):
    def __init__(self,
                 path=os.path.expanduser('datasets/font.lmdb'),
                 image_size=100,
        ...
        self.root = path
        self.original_resolution = original_resolution # change it to (32,100) or (32,128)
        self.data = BaseLMDBFont(self.root, original_resolution, zfill=5)
        self.length = len(self.data)
        
        # fontname_to_prefix mapping
        lines = open('/home/devesh_temp/scratch/str_kiran/font_translator_gan_hindi/font_translator_gan/datasets/Devesh_data/font_name_to_prefix.txt', 'r').readlines()
        self.fontname_to_prefix = {}
        for line in lines:
            # split by last space
            font, prefix = line.rsplit(' ', 1)
            self.fontname_to_prefix[font] = prefix

        # self.fontname_to_int = {'times.ttf': 0, 'Calibri_regular.ttf': 1, 'Potra.ttf': 2, 'Bidenatrial.ttf': 3, 'JacquesFrancoisShadow-Regular.ttf': 4}
        self.fontname_to_int = {}
        if split is None:
            self.offset = 0
        elif split == 'train':
            # last 60k
        ...
        self.length = self.preprocess()

    def __len__(self):
        return self.length
    
    def _create_env(self):
        return lmdb.open(self.root, max_readers=16, readonly=True, create=False,
    ...
    def preprocess(self):
        with self._create_env() as env, env.begin() as txn:
            ...
            for index in range(num_samples):
                index += 1
                img, font, label = self.data[index]
                font_prefix  = self.fontname_to_prefix[font]
                if font_prefix in self.fontname_to_int:
                    font = self.fontname_to_int[font_prefix]
                else:
                    self.fontname_to_int[font_prefix] = len(self.fontname_to_int)
                #     continue
    ...
    def __getitem__(self, index):
        ...
        index = index + self.offset
        img, font, label = self.data[index]
        # if img isn't rgb convert it to rgb
        if img.mode != 'RGB':
            img = img.convert('RGB')
        
        font = self.fontname_to_int[self.fontname_to_prefix[font]]
        if self.transform is not None:
            img = self.transform(img)
        
        return {'img': img, 'index': index, 'fontname':font, 'label':label}
