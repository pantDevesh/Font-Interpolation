...
class LitModel(pl.LightningModule):
    ...
    def encode(self, x, font, label):
        # TODO:
        assert self.conf.model_type.has_autoenc()
        cond = self.ema_model.encoder.forward(x, font, label)
        return cond
        ...
        with writer:
            for batch in tqdm(loader, total=len(loader), desc='infer'):
                with torch.no_grad():
                    ...
                    cond = model.encoder(batch['img'].to(self.device), batch['fontname'], batch['label'])
    ...
    def training_step(self, batch, batch_idx):
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode.require_dataset_infer():
                ...
                font = None; label = None
            else:
                imgs, idxs, font, label  = batch['img'], batch['index'], batch['fontname'].to(self.device), batch['label']
            ...
            if self.conf.train_mode == TrainMode.diffusion:
                ...
                t, weight = self.T_sampler.sample(len(x_start), x_start.device) # whats this?
                ...
                latent_losses = self.latent_sampler.training_losses(
                    model=self.model.latent_net, x_start=cond, t=t, font=font, label=label)
            ...
            reconstruction_loss = losses['loss'].mean()
            if 'style_loss' in losses:
                loss_style = losses['style_loss'].mean()
                alpha = 0.5
                loss = alpha * reconstruction_loss + (1-alpha) * loss_style
            else:
            ...
            if self.global_rank == 0:
                if 'style_loss' in losses:
                    self.logger.experiment.add_scalar('style_loss', losses['style_loss'],
                                                    self.num_samples)
                self.logger.experiment.add_scalar('loss', losses['loss'],
                                                  self.num_samples)
                for key in ['vae', 'latent', 'mmd', 'chamfer', 'arg_cnt', 'style_loss']:
                    if key in losses:
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx: int,
        ...
        if self.is_last_accum(batch_idx):
            ...
            # logging
            if self.conf.train_mode.require_dataset_infer():
                imgs = None; font = None; label = None
            else:
                imgs, font, label = batch['img'], batch['fontname'], batch['label']
                # print(imgs, font, label)
            self.log_sample(x_start=imgs, font=font, label=label)
    ...
    def log_sample(self, x_start, font, label):
        """
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    # # custom load checkpoint for Encoder
    # path = '/home/devesh_temp/scratch/diffae/checkpoints/font128_autoenc/autoencoder/epoch_3.ckpt'
